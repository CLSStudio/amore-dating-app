# Amore Android Beta 構建腳本 (修正版本)
# 用法: .\scripts\build_android_beta_fixed.ps1 ["release_notes"]
...
# 檢查 Firebase CLI
Write-Host "🔍 檢查工具..." -ForegroundColor Yellow
try {
...
try {
    ...
    Write-Host "Flutter 已就緒" -ForegroundColor Green
} catch {
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "⚠️  測試失敗，但繼續構建..." -ForegroundColor Yellow
}
...
if (Test-Path $ApkPath) {
    ...
    Write-Host "📤 嘗試分發 Android APK 到 Firebase App Distribution..." -ForegroundColor Blue
    
    # 使用 Firebase CLI 分發
    $distributeCommand = "firebase appdistribution:distribute `"$ApkPath`" --app `"1:380903609347:android:b24c6150c55e1fb260aa36`" --release-notes `"$ReleaseNotes`""
    
    try {
        Invoke-Expression $distributeCommand
